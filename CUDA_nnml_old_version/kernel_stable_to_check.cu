...
__global__ void MMMulDevFeed(DATA *h2h, DATA *w, DATA *biases, DATA *h2h_dest, DATA *delta, DATA *error, int row_w, int col_w, int stream_offset_y, int stream_size) {
...
__global__ void MMMulDevBack(DATA *delta_l, DATA *w, DATA *delta_l_1, DATA *h2h_l_1, int row_w, int col_w, int stream_size) {
...
void FeedAndBack(DATA *INPUT, struct host_to_dev_mem * htdm, struct dev_struct *dev_htdm, DATA *dev_error_mat, DATA *dev_error, int *nupl, int layers, cudaStream_t *streams, BOOL first_epoch) {
	...
	//Stream size and stream remainder
	int stream_size = TOTAL_PATT / NSTREAMS;
	int stream_remainder = TOTAL_PATT % NSTREAMS;
	//Grid setting
	dim3 grid, block;
	//int patt_per_step;
	//Useful pointers
	...
	for (int i = 0; i < NSTREAMS; i++) {
		...
		//Per l'ultimo stream va aggiunto il resto dei pattern (non è detto che TOTAL_PATT sia divisibile per 3)
		...
		MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, error, nupl[0], nupl[1], offset, stream_size);
		...
		for (int l = 1; l < (layers - 2); l++) {
			...
			MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, error, nupl[l], nupl[l + 1], offset, stream_size);
		}
		...
		MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, error, nupl[layers - 2], nupl[layers - 1], offset, stream_size);
		...
		for (int delta_index = (layers - 2); delta_index > 0; delta_index--) {
			...
			//Pointers set up
			MMMulDevBack << <grid, block, 0, streams[i] >> > (delta_l, w, delta_l_1, h2h, nupl[delta_index], nupl[delta_index + 1], stream_size);
		}
	}
	//**HERE**
