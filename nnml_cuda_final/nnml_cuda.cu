...
__global__ void MMMulDevFeed(DATA *, DATA *, DATA *, DATA *, DATA *, DATA*, DATA*, int, int, int, int, int, int, BOOL);
...
void HOST_init_matrices(DATA **, DATA **, int*, int, DATA, DATA, FILE *, FILE *, char*, int);
...
/*UTILITIES FROM BASIC CODE OF NNML*/
void ReadFromFile(char *, void *, int, int, int);
void Usage(char *);
void WriteBack(DATA **, DATA **, int*, int, FILE *, FILE *, char *, int);
...
int main(int argc, char *argv[]) {
	...
	DATA Eps, smallwt, alpha[1], eta[1];
	int verbose=FALSE;
    ...
    READREALFI(eta[0],"Learning rate");
    READREALFI(alpha[0],"Momentum");
	...
	int total_layers = NumHL+2;
	int *nupl = (int*)makevect(total_layers,sizeof(int));
	...
	if(NumHL) { // If there are hidden layers
      ...
      for(int i=1; i<=NumHL; i++) { // for all hidden layers
        snprintf(tempstring,sizeof(tempstring),"Number of units in layer %d",i-1);
        READINTFI(scratch,tempstring);
        nupl[i]=scratch;
      }
    }
	//Se ci sono i pesi e i deltapesi sui file rispettivamente RestartFileName e RestartDeltaFileName li usiamo
	...
	if(strcmp(LogFileName,NULLFILE)) { fpl=Fopen(LogFileName,"w"); } //File di log

	/*++++------------------------------------ERRORS--------------------------------------------------++++*/

	ERROR_MAT = (DATA*)makevect(NumPattern*NumOutput,sizeof(DATA)); // matrice di ERROR per il controllo di correttezza
	HANDLE_CUDA(cudaMalloc((void **)&DEV_ERROR_MAT, NumPattern*NumOutput * sizeof(DATA))); //matrice di ERROR del device
	...
	/*++++-------------------------------------alpha--and--eta---memcpy--to--symbol---------------------------------++++*/

	//il valore di default di cudaMemcpyKind � cudaMemcpyHostToDevice
	HANDLE_CUDA(cudaMemcpyToSymbol(alpha_const, alpha, sizeof(DATA)));
	HANDLE_CUDA(cudaMemcpyToSymbol(eta_const, eta, sizeof(DATA)));

	/*---------------------------------------end--alpha--and--eta---memcpy--to--symbol----------------------------------*/

	/*++++---------------------------init INPUT_MAT and TARGET (HOST)-----------------------------++++*/

	TARGET = (DATA*)makevect(NumPattern *NumInput,sizeof(DATA)); //TARGET dei PATTERNS

	/*INPUT_MAT is pinned memory*/

	HANDLE_CUDA(cudaHostAlloc(&INPUT_MAT, NumPattern *NumInput* sizeof(DATA), 0));

	//Lettura degli Input e dei Target dai rispettivi files
	ReadFromFile(InputFileName,INPUT_MAT,NumPattern,NumInput,sizeof(DATA));
    ReadFromFile(TargetFileName,TARGET,NumPattern,NumOutput,sizeof(DATA));
	...
	//Host 
	WeightBias = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	DeltaWeightBias = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	HOST_init_matrices(WeightBias, DeltaWeightBias, nupl, total_layers, alpha[0], smallwt, fp, fpd, formatstring, verbose);
	
	//Device
	WeightBiasDev = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	DeltaWeightBiasDev = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	H2HDev = (DATA**)Malloc((total_layers) * sizeof(DATA*));
	DeltaDev = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	DeltaDevT = (DATA**)Malloc((total_layers - 1) * sizeof(DATA*));
	DEVICE_alloc_matrices(WeightBiasDev, DeltaWeightBiasDev, H2HDev, DeltaDev, DeltaDevT, nupl, total_layers, NumPattern);
	...
	FeedAndBack(INPUT_MAT, ud, DEV_ERROR_MAT, DEV_ERROR, nupl, total_layers, NumPattern, streams, 1);
	//cudaDeviceSynchronize();//
	GradientDescent(ud, nupl, eta[0], total_layers, NumPattern, streams);
	...
	if((*ERROR) >= Eps){
		for (int epoch = 1; epoch < maxepoch; epoch++) {
			...
			startTimer(&start, &stop);
			FeedAndBack(INPUT_MAT, ud, DEV_ERROR_MAT, DEV_ERROR, nupl, total_layers, NumPattern, streams, 0);
			//cudaDeviceSynchronize();//
			GradientDescent(ud, nupl, eta[0], total_layers, NumPattern, streams);
			stopAndPrint(&start, &stop);

			HANDLE_CUDA(cudaMemcpy(ERROR, DEV_ERROR, sizeof(DATA), cudaMemcpyDeviceToHost));//La cudaMemcpy in questo caso � SINCRONA e quindi funge da punto
			printf("Reduced Error for epoch %d: %f\n\n", epoch, *ERROR);					// di sincronizzazione
			
			if(fpl) { fprintf(fpl,"Epoch %-5d :   Error = %f\n", epoch, (*ERROR)); fflush(fpl); }
			
			if((*ERROR) < Eps){break;}
		}
	}
	
	//I WeightBias e DeltaWeightBias risultanti andranno scritti nei rispettivi files
	for (int i = 0; i < (total_layers - 1); i++) {
		HANDLE_CUDA(cudaMemcpy(WeightBias[i], WeightBiasDev[i], (nupl[i] + 1)*nupl[i + 1] * sizeof(DATA), cudaMemcpyDeviceToHost));
		HANDLE_CUDA(cudaMemcpy(DeltaWeightBias[i], DeltaWeightBiasDev[i], (nupl[i] + 1)*nupl[i + 1] * sizeof(DATA), cudaMemcpyDeviceToHost));
	}

	/*---------------------------------------END--FEEDFORWARD--BACKPROPAGATION--AND--GRADIENT--DESCENT--------------------------------------*/

	/*++++---------------Write WeightBias and DeltaWeightBias back to files------------------++++*/
	
	fp=Fopen(ResultFileName,"w");
	fpd=Fopen(DeltaFileName,"w");
	WriteBack(WeightBias, DeltaWeightBias, nupl, total_layers, fp, fpd, formatstring, verbose);
	
	/*-----------------End Write WeightBias and DeltaWeightBias back to files--------------------*/
	
	/*++++--------------------------------deallocations------------------------------------++++*/
	//Deallocations
	HOST_clean(WeightBias, DeltaWeightBias, total_layers);
	free(nupl);
	free(TARGET);
	free(ERROR_MAT);
	free(ud);
	DEVICE_clean(WeightBiasDev, DeltaWeightBiasDev, H2HDev, DeltaDev, DeltaDevT, total_layers);
...
offset_y � la posizione considerata lungo le y (nelle matrici h2h, h2h_dest ed eventualmente error) durante la chiamata corrente a questo __device__.
DeltaO � calcolato per l'output layer (out_layer == 1) e propagato poi con backpropagation.
Anche error � calcolato per l'output layer (out_layer == 1) ma non viene propagato all'indietro.
*/
...
Bisogna ricordare che ogni stream che invoca questo kernel visiona una porzione dei pattern che � pari a stream_size, salvo l'aggiunta
del possibile resto della divisione total_pattern/NSTREAMS.
*/
__global__ void MMMulDevFeed(DATA *h2h, DATA *w, DATA *biases, DATA *h2h_dest, DATA *delta, DATA *deltaT, DATA *error, int row_w, int col_w, int stream_offset_y, int stream_size, int tot_patt, int neuro_out, BOOL out_layer) {

	int current_patts;
	int remaining_patts;
	int patt_per_step = gridDim.y*blockDim.y; //� il numero di pattern (quando possibile...) da considerare in ciascuna iterazione su h2h e quindi su h2h_dest
	...
	for (int y = 0; y < stream_size; y += patt_per_step) {
		...
		MMMulDevPartialFeed(h2h + y*row_w, w, biases, h2h_dest + y*col_w, delta + y*neuro_out, deltaT + y, error + y*neuro_out, row_w, col_w, current_patts, stream_offset_y + y, tot_patt, out_layer);
	}
}
...
void FeedAndBack(DATA *INPUT, util_dev *ud, DATA *dev_error_mat, DATA *dev_error, int *nupl, int layers, int patterns, cudaStream_t *streams, BOOL first_epoch) {
	...
	for (int i = 0; i < NSTREAMS; i++) {
		...
		//Per l'ultimo stream va aggiunto il resto dei pattern (non � detto che patterns sia divisibile per 3)
		...
		deltaT = (ud->DeltaDevT)[layers - 2] + offset; //Basta offset perch� � la versione trasposta di delta
		...
		MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, deltaT, error, nupl[0], nupl[1], offset, stream_size, patterns, nupl[layers-1], 0);
		...
		for (int l = 1; l < (layers - 2); l++) {
			...
			//Delta e error gi� settati
			//Puntatori settati

			MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, deltaT, error, nupl[l], nupl[l + 1], offset, stream_size, patterns, nupl[layers-1], 0);
		}
		...
		MMMulDevFeed << <grid, block, 0, streams[i] >> > (h2h, w, bias, h2h_dest, delta, deltaT, error, nupl[layers - 2], nupl[layers - 1], offset, stream_size, patterns, nupl[layers-1], 1);
		...
		for (int delta_index = (layers - 2); delta_index > 0; delta_index--) {
			...
			delta_l_1T = (ud->DeltaDevT)[delta_index - 1] + offset; //Basta offset perch� � la versione trasposta di delta_l_1
			//Puntatori settati
...
void GradientDescent(util_dev *ud, int *nupl, DATA eta, int layers, int patterns, cudaStream_t *streams) {
	...
	for (int s = 0; s < NSTREAMS; s++) {
		...
		for (int delta_index = (layers - 2); delta_index >= 0; delta_index--) {
			//Ricordare che la griglia deve 'adagiarsi' sulla matrice di output che adesso � delta_weight che ha dimensioni nupl[delta_index]
			...
			delta_bias = (ud->DeltaWeightBiasDev)[delta_index];//La prima riga di (ud->DeltaWeightBiasDev)[delta_index] � la riga dei bias
			//Puntatori settati
...
void HOST_init_matrices(DATA **WeightBias, DATA **DeltaWeightBias, int* nupl, int layers, DATA alpha, DATA smallwt, FILE *fp, FILE *fpd, char *formatstring, int verbose) {
	
	for (int i = 0; i < (layers - 1); i++) {

		WeightBias[i] = (DATA*)makevect((nupl[i] + 1)*nupl[i + 1],sizeof(DATA));
		DeltaWeightBias[i] = (DATA*)makevect((nupl[i] + 1)*nupl[i + 1],sizeof(DATA));

		for (int j = 0; j < (nupl[i] + 1); j++) {
			for (int k = 0; k < nupl[i + 1]; k++) {
				WeightBias[i][j*nupl[i + 1] + k] = 2.0 * ( drand48() - 0.5 ) * smallwt ;
	...
	if(fp != NULL){
		for( int k = 0 ; k < nupl[layers - 1] ; k++ ) {
            fscanf(fp,formatstring,&WeightBias[layers-2][0*nupl[layers-1]+k]); 
            if(fpd) {
            	fscanf(fpd,formatstring,&DeltaWeightBias[layers-2][0*nupl[layers-1]+k]);
				DeltaWeightBias[layers-2][0*nupl[layers-1]+k] *= alpha;
			}
            for( int j = 1 ; j <= nupl[layers - 2] ; j++ ) {
                fscanf(fp,formatstring,&WeightBias[layers-2][j*nupl[layers-1]+k]);
                if(fpd){
                	fscanf(fpd,formatstring,&DeltaWeightBias[layers-2][j*nupl[layers-1]+k]);
                	DeltaWeightBias[layers-2][j*nupl[layers-1]+k] *= alpha;
                } 
            }
            fscanf(fp,"\n");
            if(fpd) fscanf(fpd,"\n");
        ...
        for(int h=(layers-2); h>0; h--) {
            for( int j = 0 ; j < nupl[h] ; j++ ) {
                fscanf(fp,formatstring,&WeightBias[h-1][0*nupl[h]+j]);
                if(fpd){
                	fscanf(fpd,formatstring,&DeltaWeightBias[h-1][0*nupl[h]+j]);
                	DeltaWeightBias[h-1][0*nupl[h]+j] *= alpha;
                } 
                for( int i = 1 ; i <= nupl[h-1] ; i++ ) {
                	fscanf(fp,formatstring,&WeightBias[h-1][i*nupl[h]+j]);
                	if(fpd){
                		fscanf(fpd,formatstring,&DeltaWeightBias[h-1][i*nupl[h]+j]);
                		DeltaWeightBias[h-1][i*nupl[h]+j] *= alpha;
                	} 
                }
                fscanf(fp,"\n");
                if(fpd)fscanf(fpd,"\n");
	...
	if(verbose) {
	    ...
	    for( int k = 0 ; k <  nupl[layers-1] ; k++ ) {
	    	printf("Bias H to O[%d]: %f\n",k,WeightBias[layers-2][0*nupl[layers-1]+k]); 
	        for( int j = 1 ; j <= nupl[layers-2] ; j++ ) {           
	        	printf("Weight H[%d] to O[%d]: %f\n",j,k,WeightBias[layers-2][j*nupl[layers-1]+k]);
	        }
	    }
	    for(int h=(layers-2); h>0; h--) { 
	    	for( int j = 0 ; j < nupl[h] ; j++ ) { 
	    		printf("Bias[%d][%d]: %f\n",h-1,j,WeightBias[h-1][0*nupl[h]+j]); 
	    		for( int i = 1 ; i <= nupl[h-1] ; i++ ) { 
	    			printf("Weight[%d][%d][%d]: %f\n",h-1,i,j,WeightBias[h-1][i*nupl[h]+j]);
	    		}
	    	}
	    }
	}
...
/*I WeightBias e DeltaWeightBias risultanti andranno scritti nei rispettivi files*/
void WriteBack(DATA **WeightBias, DATA **DeltaWeightBias, int* nupl, int layers, FILE *fp, FILE *fpd, char *formatstring, int verbose){
	...
	for( int k = 0 ; k < nupl[layers-1] ; k ++ ) {
		...
		fprintf(fp,"%7.5f ",WeightBias[layers-2][0*nupl[layers-1]+k]);
		fprintf(fpd,"%g ",DeltaWeightBias[layers-2][0*nupl[layers-1]+k]);
		for( int j = 1 ; j <= nupl[layers-2] ; j++ ) {
			if(verbose) {
				printf("Weight H[%d] to O[%d]: %f\n",j,k,WeightBias[layers-2][j*nupl[layers-1]+k]);
			}
			fprintf(fp,"%7.5f ",WeightBias[layers-2][j*nupl[layers-1]+k]);
			fprintf(fpd,"%g ",DeltaWeightBias[layers-2][j*nupl[layers-1]+k]);
		}
		fprintf(fp,"\n");
		fprintf(fpd,"\n");
	}
	...
	for(int h=(layers-2); h>0; h--) {
		for( int j = 0 ; j < nupl[h] ; j++ ) {
			if(verbose) {
				printf("BiasH2H[%d][%d]: %f\n",h,j,WeightBias[h-1][0*nupl[h]+j]);
	        }
	        fprintf(fp,"%7.5f ",WeightBias[h-1][0*nupl[h]+j]);
	        fprintf(fpd,"%g ",DeltaWeightBias[h-1][0*nupl[h]+j]);
	        for( int i = 1 ; i <= nupl[h-1] ; i++ ) {
	        	if(verbose) {
	        		printf("WeightH2H[%d][%d] to H{%d]: %f\n",h,i,j,WeightBias[h-1][i*nupl[h]+j]);
	        	}
	        	fprintf(fp,"%7.5f ",WeightBias[h-1][i*nupl[h]+j]);
	        	fprintf(fpd,"%g ",DeltaWeightBias[h-1][i*nupl[h]+j]);
	        }
	        fprintf(fp,"\n");
	        fprintf(fpd,"\n");
		}
